...
const RenderMember = ({member : {name, email, linkedin, photoKey, position, team}, isLead}) => (
  <div className={`member flex flex-col ${isLead ? 'w-[310px]' : 'w-[195px]'} justify-between`}>
    <div className={`photo ${isLead ? 'w-[310px] h-[300px]' : 'w-[190px] h-[190px]'}`}>
...
const RenderTeams = () => (
  <div className={'team'}>
    <div className={'leads flex flex-row justify-center mb-4 gap-10'}>
    {
      memberJson.leads.map(lead => <RenderMember member={lead} isLead={true}/>)
    }
    </div>
    <div className={'vps flex flex-row justify-between mb-4'}>
    {
    ...
    </div>
    {
      memberJson.teams.map(team => team.members.map(mem => ({...mem, team:team.name}))).map(team => (
        <div className='team flex flex-row justify-start mb-4'>
        {
